...
const TransferFund = ({account, getBalance}) => {
    ...
    return(
        <div>
            <br/>
            <BodyText>Buy Airtime</BodyText>
            <FormStyle onChange = {(e) => receiverAddress.current = e.target.value} placeholder="Receiver Mobile Number" /><br/>
            <FormStyle onChange = {(e) => transferAmount.current = e.target.value} placeholder="Amount" /><br/>
            <TransactionButton onClick ={transferFund}>{isLoading ? "loading...": "Buy Airtime"}</TransactionButton>
        </div>
    )
}
